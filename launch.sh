...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "dm-sb-kie-client-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name dm-sb-kie-client-service apps/dm-sb-kie-client-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app dm-sb-kie-client-service:1.0-SNAPSHOT
    oc expose svc/dm-sb-kie-client-service
else

	echo "Launching the application locally..."
	pattern="dm-sb-kie-client-service"
	files=( $pattern )
	cd ${files[0]}
